...
async function getAllHistoricalRecords() {
    try {
        const result = await client.query('SELECT * FROM historical_spx;');
        return result.rows;
    } catch (error) {
...
async function getHistoricalRecordById(id) {
    try {
        const result = await client.query('SELECT * FROM historical_spx WHERE id = $1;', [id]);
        return result.rows[0];
    } catch (error) {
...
async function deleteHistoricalRecord(id) {
    try {
        const result = await client.query('DELETE FROM historical_spx WHERE id = $1 RETURNING *;', [id]);
        return result.rows[0];
    } catch (error) {
