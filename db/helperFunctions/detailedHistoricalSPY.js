...
// Create a record in detailed_historical_spy
async function createDetailedRecord({ timestamp, price, volume }) {
    try {
        const result = await client.query(`
            INSERT INTO detailed_historical_spy (timestamp, price, volume)
            VALUES ($1, $2, $3)
            RETURNING *;
...
// Get all records from detailed_historical_spy
async function getAllDetailedRecords() {
    try {
        const result = await client.query('SELECT * FROM detailed_historical_spy ORDER BY timestamp ASC;');
        return result.rows;
    } catch (error) {
...
// Get a record by ID from detailed_historical_spy
async function getDetailedRecordById(id) {
    try {
        const result = await client.query('SELECT * FROM detailed_historical_spy WHERE id = $1;', [id]);
        return result.rows[0];
    } catch (error) {
...
// Update a record in detailed_historical_spy
async function updateDetailedRecord(id, fields = {}) {
    ...
    try {
        const result = await client.query(`
            UPDATE detailed_historical_spy
            SET ${setString}
...
// Delete a record from detailed_historical_spy
async function deleteDetailedRecord(id) {
    try {
        const result = await client.query('DELETE FROM detailed_historical_spy WHERE id = $1 RETURNING *;', [id]);
        return result.rows[0];
    } catch (error) {
